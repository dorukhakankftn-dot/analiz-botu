...
from config import Config, TF_LA_BASE, TF_RAY_LEVELS, TF_SR, TF_L3
...
class StructureEngine:
    ...
    def build_l4(self, candles: list[Candle], tf: str) -> list[DrawStructure]:
        # c1.close ~ c2.open (tolerans icinde ama tam esit degil) -> gercek egimli isin.
        # c2.open < c1.close ise dusus varyanti olusuyor, bu ~%90 teyitli kabul edilir.
        out = []
        for i in range(1, len(candles)):
            c1, c2 = candles[i - 1], candles[i]
            if c1.is_green and c2.is_green:
                if abs(c2.open - c1.close) <= c1.close * self.tol_l4:
                    out.append(DrawStructure(
                        "L4", "ray", tf, c2.open,
                        slope_per_sec=self._slope(c1, c1.close, c2, c2.open),
                        anchor_time=c2.close_time,
                        high_confidence=(c2.open < c1.close),
                    ))
        return out
    ...
    def build_sr(self, candles: list[Candle], tf: str) -> list[DrawStructure]:
        # iki ardisik YESIL mum (kirmizi sarti yok). LSU ve LSD birbirinden
        # BAGIMSIZ sartlara sahip - biri saglaniyor diye digeri otomatik
        # cizilmez, her biri kendi dusus kosulunu ayri ayri saglamali.
        out = []
        for i in range(1, len(candles)):
            c1, c2 = candles[i - 1], candles[i]
            if not (c1.is_green and c2.is_green):
                continue
            if c2.high < c1.high:
                # ust fitilde dusus -> "pat pat" miknatis davranisli
                out.append(DrawStructure(
                    "LSU", "ray", tf, c2.high,
                    slope_per_sec=self._slope(c1, c1.high, c2, c2.high),
            ...
            if c2.low < c1.low:
                # alt fitilde dusus -> pasif referans, miknatis yok
                out.append(DrawStructure(
                    "LSD", "ray", tf, c2.low,
                    slope_per_sec=self._slope(c1, c1.low, c2, c2.low),
    ...
    @staticmethod
    def _is_expired_by_atl(struct: DrawStructure, atl: float, now_ts: float) -> bool:
        """Egimli bir isin, ATL sabit cizgisiyle gelecekte kesisecekse ve o
        zaman gelip gectiyse hukmu biter. Yatay yapilar (slope=0) ve ATL\'den
        uzaklasan isinlar asla suresi dolmaz."""
        if not struct.slope_per_sec or atl is None:
            return False
        t_cross = struct.anchor_time + (atl - struct.price) / struct.slope_per_sec
        if t_cross <= struct.anchor_time:
            return False  # kesisim gecmiste / bu yon ATL\'ye yaklasmiyor
        return now_ts >= t_cross
    ...
    def build_new_coin(self, candles: list[Candle], tf: str) -> list[DrawStructure]:
        # coin\'in hayatindaki ilk 2 mum (ikisi de yesil olmali), sadece "yeni coin"
        # penceresi (Config.NEW_COIN_MAX_DAYS) boyunca cagirilir; referans olarak
        # her zaman gercek ilk 2 muma bakar, o anki son 2 muma degil.
        out = []
        if len(candles) < 2:
            return out
    ...
    def build_all(self, candles_by_tf: dict[str, list[Candle]], is_new_coin: bool) -> list[DrawStructure]:
        ...
        atl = min(c.low for c in monthly) if monthly else None
        if atl is not None:
        ...
        # ATL kesisim-son kullanma kurali: egimli tum isinlar (L1,L2,L4,L5,
        # LSU,LSD) icin gecerli, yatay yapilar (LA,L3,LF,LG,LN) etkilenmez.
        now_ts = time.time()
        if atl is not None:
            structures = [s for s in structures if not self._is_expired_by_atl(s, atl, now_ts)]
    ...
    def find_candidate(candles_signal_tf: list[Candle], structures: list[DrawStructure], now_ts: float):
        ...
        if buy_candidates:
            ...
            sl = lvl * (1 - Config.SL_PERCENT / 100)
